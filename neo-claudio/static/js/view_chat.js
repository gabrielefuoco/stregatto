...
import { CatClient } from './cat_client.js?v=2';
...
class MCPHostProxy {
    constructor(iframeElement, wsUrl) {
        ...
        this.iframeListener = this.handleIframeMessage.bind(this);
        window.addEventListener('message', this.iframeListener);
        
        // Let the iframe know it's ready and sync theme
    ...
    container.innerHTML = `
        ...
        <div class="flex-1 flex overflow-hidden relative z-10">
            ...
            <div class="flex-1 flex flex-col relative min-w-0 h-full overflow-hidden">
                ...
                <!-- Input Area -->
                <div class="w-full max-w-4xl mx-auto px-lg pb-md pt-xs shrink-0 relative">
                    <!-- Slash Commands Menu Pop-up -->
                    <div id="slash-menu" class="absolute bottom-full left-lg mb-xs w-80 bg-surface-container-lowest border-2 border-on-surface shadow-[4px_4px_0px_0px_#1a1c1c] z-50 hidden flex-col">
                        <div class="px-sm py-xs border-b-2 border-on-surface bg-surface-container font-mono text-[11px] font-bold text-secondary uppercase tracking-wider flex justify-between items-center select-none">
                            <span>Comandi Rapidi (Slash)</span>
                            <kbd class="text-[9px] bg-surface px-1 border border-on-surface font-mono">ESC</kbd>
                        </div>
                        <div id="slash-items-list" class="flex flex-col max-h-56 overflow-y-auto">
                        </div>
                    </div>
                    ...
                    <form id="chat-form" class="relative flex items-end bg-surface-container-lowest border-2 border-on-surface shadow-[4px_4px_0px_0px_#ff5f1f] focus-within:shadow-[4px_4px_0px_0px_#1a1c1c] transition-all p-sm mb-xs">
                        ...
                        <!-- Send / Stop Button -->
                        <button type="submit" id="btn-submit" class="p-sm bg-primary-container text-on-primary border-2 border-on-surface shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-x-[1px] active:translate-y-[1px] active:shadow-none transition-all ml-xs flex items-center justify-center h-[44px] w-[44px] shrink-0" title="Invia">
            ...
            <aside id="right-sidebar" class="flex flex-col h-full sidebar-transition border-l-2 border-on-surface bg-cream-bg shrink-0 z-20 shadow-[0_4px_6px_-1px_rgba(0,0,0,0.1)] relative artifacts-collapsed">
                <!-- Resizer Handle -->
                ...
                <!-- Code Viewer Section -->
                <div class="flex-1 flex flex-col overflow-hidden bg-cream-bg relative">
                    ...
                    <div id="mcp-app-host" class="absolute inset-0 bg-surface hidden flex-col z-20 border-b-2 border-on-surface"></div>
                </div>
...
function initChatLogic(container) {
    ...
    const mcpAppHost = container.querySelector('#mcp-app-host');
    ...
    const mcpUrl = `${wsProto}//${window.location.host}/mcp_client/ws/mcp-events`;
    catClient.connectMCP(mcpUrl).catch(err => console.error("Failed to connect to MCP events:", err));

    catClient.on('mcp_event', async (data) => {
        if (data.type === 'app_launch' && data.uri) {
            openSidebar();
            const currentCanvasTitle = document.getElementById('canvas-title');
            const currentMcpAppHost = document.getElementById('mcp-app-host');

            if (currentCanvasTitle) currentCanvasTitle.innerHTML = `<span class="material-symbols-outlined text-sm">web</span> App Loading`;
            
            if (currentMcpAppHost) {
                currentMcpAppHost.classList.remove('hidden');
                currentMcpAppHost.classList.add('flex');
                currentMcpAppHost.style.display = 'flex';
                
                currentMcpAppHost.innerHTML = `
                    <div class="flex flex-col items-center justify-center h-full text-secondary">
                        ...
                        <p class="mt-4 font-mono text-sm">Caricamento risorsa ${data.uri}...</p>
                    </div>
                `;
                
                try {
                    const serverName = encodeURIComponent(data.server_name || '');
                    const uri = encodeURIComponent(data.uri);
                    const session = await getSession();
                    const localToken = localStorage.getItem('stregatto_auth_token');
                    const token = session ? session.access_token : localToken;
                    const headers = token ? { 'Authorization': `Bearer ${token}` } : {};

                    const htmlRes = await fetch(`/api/mcp/resource?server_name=${serverName}&uri=${uri}`, { headers });
                    ...
                    iframe.srcdoc = html;
                    currentMcpAppHost.appendChild(iframe);
                    
                    if (currentCanvasTitle) currentCanvasTitle.innerHTML = `<span class="material-symbols-outlined text-sm">web</span> ${data.server_name || 'App'}`;

                    const proxyUrl = `${wsProto}//${window.location.host}/mcp_client/ws/mcp-proxy/${serverName}`;
                    new MCPHostProxy(iframe, proxyUrl);

                    iframe.addEventListener('load', () => {
                        if (iframe.contentWindow) {
                            // 1. Invia gli argomenti del tool (elements) — triggera il rendering SVG
                            if (data.tool_arguments) {
                                iframe.contentWindow.postMessage({
                                    jsonrpc: "2.0",
                                    method: "ui/notifications/tool-input",
                                    params: data.tool_arguments
                                }, '*');
                            }
                            // 2. Invia il risultato strutturato (checkpointId) — abilita editing/persistenza
                    ...
                    console.error("Errore caricamento MCP App", err);
                    mcpAppHost.innerHTML = `<div class="p-4 text-red-500">Errore: ${err.message}</div>`;
                }
            }
        }
    });
    ...
    if (modelSelect) {
        CatAPI.getModels().then(models => {
            if (models && models.length > 0) {
                ...
                } else if (models[0] && models[0].id) {
                    modelSelect.value = models[0].id;
                    localStorage.setItem('stregatto_selected_model', models[0].id);
                }
            }
    ...
    function openSidebar() {
        const activeRightSidebar = document.getElementById('right-sidebar');
        if (activeRightSidebar) {
            activeRightSidebar.classList.remove('hidden');
            activeRightSidebar.classList.remove('artifacts-collapsed');
            activeRightSidebar.classList.add('flex');
            activeRightSidebar.classList.add('artifacts-expanded');
            activeRightSidebar.style.display = 'flex';
            const w = localStorage.getItem('stregatto_right_sidebar_width') || '550px';
            activeRightSidebar.style.width = w;
            activeRightSidebar.style.minWidth = w;
        }
    }
            ...
            updateThinkingLoader: (tokens) => {
                ...
                if (activeTextStep && activeTextStep.body) {
                    ...
                    activeTextStep.body.innerHTML = `
                        <div class="flex items-center gap-2 p-xs font-mono text-xs text-on-surface bg-surface-variant border-2 border-on-surface shadow-[2px_2px_0px_0px_#1a1c1c]">
                            <span class="animate-pulse">🧠</span>
                            <span class="font-bold uppercase">Ragionamento in corso...</span>
                            <span class="font-bold text-[#FF5F1F]">${tokens} token</span>
                        </div>
                    `;
                }
                messagesContainer.scrollTop = messagesContainer.scrollHeight;
    ...
    function setSubmitButtonState(streaming) {
        isStreaming = streaming;
        if (!btnSubmit) return;
        if (streaming) {
            btnSubmit.className = "px-md bg-[#FF5F1F] text-black border-2 border-on-surface shadow-[2px_2px_0px_0px_#1a1c1c] font-bold font-mono text-xs uppercase flex items-center justify-center gap-xs h-[44px] shrink-0 active:translate-x-[1px] active:translate-y-[1px] active:shadow-none transition-all cursor-pointer";
            btnSubmit.title = "Ferma generazione (Esc)";
            btnSubmit.type = "button";
            btnSubmit.innerHTML = `<span class="material-symbols-outlined text-sm">stop_circle</span> <span class="font-bold">STOP</span>`;
        } else {
            btnSubmit.className = "p-sm bg-primary-container text-on-primary border-2 border-on-surface shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:translate-x-[1px] active:translate-y-[1px] active:shadow-none transition-all ml-xs flex items-center justify-center h-[44px] w-[44px] shrink-0 cursor-pointer";
            btnSubmit.title = "Invia";
            btnSubmit.type = "submit";
            btnSubmit.innerHTML = `<span class="material-symbols-outlined" style="font-variation-settings: 'FILL' 1;">send</span>`;
        }
    }

    async function cancelCurrentRun() {
        if (!isStreaming) return;
        logTerminal("Interruzione generazione in corso...");
        ...
        try {
            await CatAPI.cancelAgentMessage(agentSlug, {
                session_id: currentClaudeSessionId,
                chat_id: currentChatId
            });
        } catch (err) {
            console.warn("Cancel request error:", err);
        }
        logTerminal("Generazione interrotta dall'utente.");
    ...
    const SLASH_COMMANDS = [
        { cmd: '/clear', desc: 'Pulisce i messaggi visivi nella chat', action: 'clear' },
        { cmd: '/compact', desc: 'Sintetizza i punti chiave della conversazione', action: 'compact' },
        { cmd: '/model', desc: 'Cambia modello LLM selezionato', action: 'model' },
        { cmd: '/cost', desc: 'Mostra token e costi della sessione', action: 'cost' },
        { cmd: '/file', desc: 'Allega un file dal filesystem', action: 'file' },
        { cmd: '/help', desc: 'Mostra la guida rapida ai comandi', action: 'help' }
    ];

    const slashMenu = container.querySelector('#slash-menu');
    const slashItemsList = container.querySelector('#slash-items-list');
    ...
    function hideSlashMenu() {
        if (slashMenu) {
            ...
            slashMenu.classList.remove('flex');
        }
    }

    function renderSlashMenu() {
        if (!slashItemsList) return;
        slashItemsList.innerHTML = filteredSlashCommands.map((item, index) => {
            const isSelected = index === slashSelectedIndex;
            const bgClass = isSelected ? 'bg-[#FF5F1F] text-black font-bold' : 'hover:bg-surface-variant text-on-surface';
            return `
                <div class="slash-item px-sm py-xs cursor-pointer border-b border-on-surface/20 flex items-center justify-between text-xs transition-colors ${bgClass}" data-index="${index}">
                    <span class="font-mono font-bold">${item.cmd}</span>
                    <span class="text-[11px] opacity-80 truncate ml-sm">${item.desc}</span>
                </div>
            `;
        }).join('');

        slashItemsList.querySelectorAll('.slash-item').forEach(el => {
            el.addEventListener('click', () => {
                const idx = parseInt(el.dataset.index);
                if (filteredSlashCommands[idx]) {
                    executeSlashCommand(filteredSlashCommands[idx]);
                }
            });
        });
    }
    ...
    function checkSlashMenu() {
        ...
        if (match) {
            ...
            filteredSlashCommands = SLASH_COMMANDS.filter(c => c.cmd.toLowerCase().includes('/' + query));
            if (filteredSlashCommands.length > 0) {
                if (slashSelectedIndex >= filteredSlashCommands.length) slashSelectedIndex = 0;
                renderSlashMenu();
                slashMenu.classList.remove('hidden');
                slashMenu.classList.add('flex');
                return;
            }
        }
        hideSlashMenu();
    ...
    function executeSlashCommand(item) {
        ...
        if (item.action === 'clear') {
            currentMessages = [];
            const msgContainer = container.querySelector('#messages-container');
            if (msgContainer) {
            ...
            chatInput.value = '';
            logTerminal('Chat pulita con /clear');
        } else if (item.action === 'compact') {
            chatInput.value = 'Sintetizza i punti chiave della conversazione e compatta la memoria.';
            chatForm?.dispatchEvent(new Event('submit', { cancelable: true, bubbles: true }));
        } else if (item.action === 'model') {
            chatInput.value = '';
            if (modelSelect) modelSelect.focus();
        } else if (item.action === 'cost') {
            chatInput.value = '';
            const usage = lastRunUsage || {};
            const inTok = usage.input_tokens || 0;
            const outTok = usage.output_tokens || 0;
            const costUSD = usage.total_cost_usd || 0;
            alert(`📊 Token & Costi Sessione:\n\nInput Tokens: ${inTok}\nOutput Tokens: ${outTok}\nCosto totale: $${costUSD.toFixed(6)} USD`);
            logTerminal(`Costo sessione: $${costUSD.toFixed(6)} USD (In: ${inTok}, Out: ${outTok})`);
            ...
            alert(`ℹ️ Guida Slash Commands:\n\n${helpText}`);
            logTerminal('Mostrata guida slash commands');
        }
    }
    ...
    async function executeAssistantRun(responseBox) {
        ...
        try {
            ...
            await CatAPI.streamMessage(sanitizedStreamMessages, agentSlug, (event) => {
                if (event.type === 'TEXT_MESSAGE_CONTENT' || event.type === 'TEXT_MESSAGE_CHUNK') {
                ...
                } else if (event.type === 'THINKING_TOKENS') {
                    responseBox.updateThinkingLoader(event.tokens || 0);
                    ...
                    if (event.session_id) {
                        currentClaudeSessionId = event.session_id;
                    }
                    if (event.usage) {
                        lastRunUsage = event.usage;
                    }
                ...
                extraPayload: {
                    model: (modelSelect?.value && modelSelect.value !== 'default') ? modelSelect.value : null,
                    chat_id: currentChatId
                }
            });
            ...
            if (err.name === 'AbortError') {
                logTerminal("Richiesta annullata dall'utente.");
            } else {
    ...
    chatInput?.addEventListener('input', checkSlashMenu);
